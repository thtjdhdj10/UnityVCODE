...
public class VEasyCalculator
{
    ...
    public static float MinMax(float origin, float min, float max)
    {
        return Mathf.Min(Mathf.Max(origin, min), max);
    }

    public static int MinMax(int origin, int min, int max)
    {
        return Mathf.Min(Mathf.Max(origin, min), max);
    }
    ...
    //public static WorldGeneral.Direction CheckTerritory(Unit a)
    //{
    //    Vector2 pos = a.transform.position;
    //    Rect rect = CameraManager.manager.GetLogicalRect();
    ...
    //                if (pos.x - a.colCircle < rect.xMin)
    //                {
    //                    return WorldGeneral.Direction.LEFT;
    //                }

    //                if (pos.x + a.colCircle > rect.xMax)
    //                {
    //                    return WorldGeneral.Direction.RIGHT;
    //                }

    //                if (pos.y - a.colCircle < rect.yMin)
    //                {
    //                    return WorldGeneral.Direction.DOWN;
    //                }

    //                if (pos.y + a.colCircle > rect.yMax)
    //                {
    //                    return WorldGeneral.Direction.UP;
    //                }
    //            }
    //            break;
    //        case Unit.ColliderType.RECT:
    ...
    //    return WorldGeneral.Direction.NONE;
    //}
    ...
    {
        NormalizedDirection(ref from);
        NormalizedDirection(ref to);
    ...
    public static void NormalizedDirection(ref float degrees)
    {
        if (degrees < 0f)
            degrees += 360f;
    ...
    public static float NonOverAtoB(float A, float B, float delta)
    {
        if(A < B)
        {
            return Mathf.Min(B, A + delta);
        }
        else if(A > B)
        {
            return Mathf.Max(B, A + delta);
        }
        else
        {
            return B;
    ...
    public static float EstimateCollidingDirection(float dir, float prevDir, float analyzingTime, float collisionTime)
    {
        ...
        NormalizedDirection(ref estimatedDirection);
